# Hand-authored initial migration (TECHNICAL.md §5).
#
# This repo is developed without a local Alliance Auth install, so this file was
# written by hand to mirror ``whatax/models.py`` rather than generated by
# ``makemigrations``. Before relying on it, run in a real AA env:
#
#     python manage.py makemigrations whatax --check --dry-run
#
# If that reports drift, delete this file and run ``makemigrations whatax`` to
# regenerate it from the models (the models are the source of truth).
